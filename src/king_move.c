    ...
    if (current_x == to_x && to_y == current_y + 2) {
        if ((player == 1 && current_x == 7 && current_y == 4) || (player == 2 && current_x == 0 && current_y == 4)) {
            ...
            for (int i=0; savegame[i]!='\0' ;i++) {
                if (player == 1 && (savegame[i+1] == '1' && (savegame[i] == 'H' || savegame[i] == 'E'))) return;
                else if (player == 2 && (savegame[i+1] == '8' && (savegame[i] == 'H' || savegame[i] == 'E'))) return;
            }
            if (warning(player , current_x, current_y+1, board,NULL,NULL) || warning(player , current_x, current_y+2, board,NULL,NULL)) return;
            else {
            ...
            for(int b =-1 ;b<2;b++){
                if (a!=0||b!=0){
                    if(current_x+a==to_x&&current_y+b==to_y){
                            if(!warning(player ,to_x,to_y,board,NULL,NULL)){
                                *flag=true;
                            }
                    }
                }
